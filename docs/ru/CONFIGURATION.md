<h1 align="center">Конфигурационные Файлы</h1>
...
1. Убеждаемся, что Hexo версии 4.3 (или выше).
2. Проверяем на существование `/source/_data/next.yml` файл (удаляем, если существует).
3. Копируем необходимые опции из конфига темы NexT `/themes/next/_config.yml` в `/_config.yml`, затем\
...
<h2 align="center">Способ 2: NexT-Путь (Не рекомендуется)</h2>
